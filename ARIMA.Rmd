...
NOTE:
Please don't run SMLA.Rmd and ARIMA.Rmd at the same time. The functions in "igraph" package in SMLA will override the functions in base R, causing ARIMA.Rmd to not work properly.


Load packages
```{r}
pacman::p_load(astsa,tseries,forecast,strucchange,readr,dplyr,patchwork,pROC,outliers,ggplot2,yardstick,scales)
```
Date time function
```{r}
...
roc_curve_cs2 <- roc(cs2_ts_date,cs2_ts_res)
 plot(roc_curve_cs2, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.819
dat <- data.frame(truth = as.factor(cs2_ts_date), pred_1 = as.numeric(cs2_ts_res), pred_2 = 1-as.numeric(cs2_ts_res))

pr_dat_cs2 <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_cs2 %>%
  arrange(.threshold) %>% 
  ggplot() +
...
```
```{r}
#from F1 score get the optimal ARIMA threshold
F1 <- 2*(pr_dat_cs2$recall*pr_dat_cs2$precision)/(pr_dat_cs2$recall+pr_dat_cs2$precision)
thre <- pr_dat_cs2$.threshold[which.max(F1)]
normalized_thre <- (thre-min(cs2_ts_res))/(max(cs2_ts_res)-min(cs2_ts_res))
cs2_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_cs2$recall[which.max(F1)],
  precision =pr_dat_cs2$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
cs2_model
```
...
example anomaly inference
```{r}
no_na <- na.omit(cs2_ts_res)
min_value <- min(no_na)
max_value <- max(no_na)
normalized <- (cs2_ts_res  - min_value) / (max_value - min_value)

positions <- which(normalized > 0.001564933)
positions_min <- 3*positions-2
positions_max <- 3*positions
time_min <- cs2_unpropogated$time[positions_min]
time_max <- cs2_unpropogated$time[positions_max]
```

```{r}
cs2_unpropogated[1:244,] %>% 
  ggplot() + 
  geom_line(aes(x=time,y=brouwer_mean_motion),col='black',size=1)+
  geom_rect(xmin=time_min[1],xmax=time_max[1],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[2],xmax=time_max[2],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[3],xmax=time_max[3],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[4],xmax=time_max[4],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[5],xmax=time_max[5],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[6],xmax=time_max[6],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[7],xmax=time_max[7],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[8],xmax=time_max[8],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[9],xmax=time_max[9],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[10],xmax=time_max[10],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[11],xmax=time_max[11],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[12],xmax=time_max[12],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[13],xmax=time_max[13],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[14],xmax=time_max[14],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[15],xmax=time_max[15],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[16],xmax=time_max[16],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[17],xmax=time_max[17],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[18],xmax=time_max[18],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_vline(xintercept = cs2_manoeuvres$time_med[1:17], col = "darkgreen",size=0.7)+
  labs(x = "Time",
       y="Unpropogated Data",
       color = "Legend") +
  ggtitle("ARIMA anomaly detection plot: CryoSat-2")+
...
roc_curve_s3a <- roc(s3a_ts_date,s3a_ts_res)
plot(roc_curve_s3a, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.801
dat <- data.frame(truth = as.factor(s3a_ts_date), pred_1 = as.numeric(s3a_ts_res), pred_2 = 1-as.numeric(s3a_ts_res))

pr_dat_s3a <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_s3a %>%
...
```
```{r}
F1 <- 2*(pr_dat_s3a$recall*pr_dat_s3a$precision)/(pr_dat_s3a$recall+pr_dat_s3a$precision)
thre <- pr_dat_s3a$.threshold[which.max(F1)]
normalized_thre <- (thre-min(s3a_ts_res))/(max(s3a_ts_res)-min(s3a_ts_res))
s3a_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_s3a$recall[which.max(F1)],
  precision =pr_dat_s3a$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
s3a_model
```
example anomaly inference
```{r}
no_na <- na.omit(final_full_res)
min_value <- min(no_na)
max_value <- max(no_na)
normalized <- (final_full_res  - min_value) / (max_value - min_value)
```


```{r}
positions <- which(normalized > normalized_thre)
positions_min <- 3*positions-2
positions_max <- 3*positions
time_min <- s3a_unpropogated$time[positions_min]
time_max <- s3a_unpropogated$time[positions_max]
```

```{r}
s3a_unpropogated[1:420,] %>% 
  ggplot() + 
  geom_line(aes(x=time,y=brouwer_mean_motion),col='black',size=1)+
  geom_rect(xmin=time_min[1],xmax=time_max[1],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[2],xmax=time_max[2],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  ...
  geom_rect(xmin=time_min[17],xmax=time_max[17],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_rect(xmin=time_min[18],xmax=time_max[18],ymin=-Inf,ymax=Inf, fill='#ad53b0',alpha = .01)+
  geom_vline(xintercept = s3a_manoeuvres$time_med[5:30], col = "darkgreen")+
  labs(x = "Time",
       y="Unpropogated Data",
       color = "Legend")+
  ggtitle("ARIMA anomaly detection plot: Sentinel-3A")
```
...
roc_curve_h2a <- roc(h2a_ts_date,h2a_ts_res)
plot(roc_curve_h2a, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.570
dat <- data.frame(truth = as.factor(h2a_ts_date), pred_1 = as.numeric(h2a_ts_res), pred_2 = 1-as.numeric(h2a_ts_res))

pr_dat_h2a <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_h2a %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
  geom_point(x=0.020,y=0.333,col="red",pch=8)+
  geom_point(x=0.56,y=0.203,col="red",pch=8)+
  coord_equal()

pr_auc(dat, truth, pred_1,event_level = "second")
```
```{r}
F1 <- 2*(pr_dat_h2a$recall*pr_dat_h2a$precision)/(pr_dat_h2a$recall+pr_dat_h2a$precision)
thre <- pr_dat_h2a$.threshold[which.max(F1)]
normalized_thre <- (thre-min(h2a_ts_res))/(max(h2a_ts_res)-min(h2a_ts_res))
h2a_model <- tibble(
  threshold = normalized_thre,
  recall = pr_dat_h2a$recall[which.max(F1)],
  precision =pr_dat_h2a$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
h2a_model
```
...
roc_curve_s3b <- roc(final_date_result,final_full_res)
plot(roc_curve_s3b, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.751
dat <- data.frame(truth = as.factor(s3b_ts_date), pred_1 = as.numeric(s3b_ts_res), pred_2 = 1-as.numeric(s3b_ts_res))

pr_dat_s3b <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_s3b %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
  geom_point(x=0.649,y=0.727,col="red",pch=8)+
  geom_point(x=0.703,y=0.413,col="red",pch=8)+
  coord_equal()

pr_auc(dat, truth, pred_1,event_level = "second")
```
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_s3b$recall*pr_dat_s3b$precision)/(pr_dat_s3b$recall+pr_dat_s3b$precision)
thre <- pr_dat_s3b$.threshold[which.max(F1)]
normalized_thre <- (thre-min(s3b_ts_res))/(max(s3b_ts_res)-min(s3b_ts_res))
s3b_model <- tibble(
  threshold = normalized_thre,
  recall = pr_dat_s3b$recall[which.max(F1)],
  precision =pr_dat_s3b$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
s3b_model
```
...
roc_curve_s6a <- roc(s6a_ts_date,s6a_ts_res )
plot(roc_curve_s6a, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.637
dat <- data.frame(truth = as.factor(s6a_ts_date), pred_1 = as.numeric(s6a_ts_res), pred_2 = 1-as.numeric(s6a_ts_res))

pr_dat_s6a <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_s6a %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_s6a$recall*pr_dat_s6a$precision)/(pr_dat_s6a$recall+pr_dat_s6a$precision)
thre <- pr_dat_s6a$.threshold[which.max(F1)]
normalized_thre <- (thre-min(s6a_ts_res))/(max(s6a_ts_res)-min(s6a_ts_res))
s6a_model <- tibble(
  threshold = normalized_thre,
  recall = pr_dat_s6a$recall[which.max(F1)],
  precision =pr_dat_s6a$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
s6a_model
```
...
roc_curve_ja1 <- roc(ja1_ts_date,ja1_ts_res)
plot(roc_curve_ja1, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.650
dat <- data.frame(truth = as.factor(ja1_ts_date), pred_1 = as.numeric(ja1_ts_res), pred_2 = 1-as.numeric(ja1_ts_res))

pr_dat_ja1 <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_ja1 %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_ja1$recall*pr_dat_ja1$precision)/(pr_dat_ja1$recall+pr_dat_ja1$precision)
thre <- pr_dat_ja1$.threshold[which.max(F1)]
normalized_thre <- (thre-min(ja1_ts_res))/(max(ja1_ts_res)-min(ja1_ts_res))
ja1_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_ja1$recall[which.max(F1)],
  precision =pr_dat_ja1$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
ja1_model
```
...
roc_curve_ja2 <- roc(ja2_ts_date,ja2_ts_res)
plot(roc_curve_ja2, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.796
dat <- data.frame(truth = as.factor(ja2_ts_date), pred_1 = as.numeric(ja2_ts_res), pred_2 = 1-as.numeric(ja2_ts_res))

pr_dat_ja2 <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_ja2 %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_ja2$recall*pr_dat_ja2$precision)/(pr_dat_ja2$recall+pr_dat_ja2$precision)
thre <- pr_dat_ja2$.threshold[which.max(F1)]
normalized_thre <- (thre-min(ja2_ts_res))/(max(ja2_ts_res)-min(ja2_ts_res))
ja2_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_ja2$recall[which.max(F1)],
  precision =pr_dat_ja2$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
ja2_model
```
...
roc_curve_ja3 <- roc(ja3_ts_date,ja3_ts_res)
plot(roc_curve_ja3, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.888
dat <- data.frame(truth = as.factor(ja3_ts_date), pred_1 = as.numeric(ja3_ts_res), pred_2 = 1-as.numeric(ja3_ts_res))

pr_dat_ja3 <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_ja3 %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_ja3$recall*pr_dat_ja3$precision)/(pr_dat_ja3$recall+pr_dat_ja3$precision)
thre <- pr_dat_ja3$.threshold[which.max(F1)]
normalized_thre <- (thre-min(ja3_ts_res))/(max(ja3_ts_res)-min(ja3_ts_res))
ja3_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_ja3$recall[which.max(F1)],
  precision =pr_dat_ja3$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
ja3_model
```
...
ROC curve
```{r}
roc_curve_fy2d <- roc(final_date_result,final_full_res)
plot(roc_curve_fy2d, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.930
dat <- data.frame(truth = as.factor(final_date_result), pred_1 = as.numeric(final_full_res), pred_2 = 1-as.numeric(final_full_res))

pr_dat_fy2d <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_fy2d %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_fy2d$recall*pr_dat_fy2d$precision)/(pr_dat_fy2d$recall+pr_dat_fy2d$precision)
thre <- pr_dat_fy2d$.threshold[which.max(F1)]
normalized_thre <- (thre-min(final_full_res,na.rm = TRUE))/(max(final_full_res,na.rm = TRUE)-min(final_full_res,na.rm = TRUE))
fy2d_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_fy2d$recall[which.max(F1)],
  precision =pr_dat_fy2d$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
fy2d_model
```
...
ROC curve
```{r}
roc_curve_fy2e <- roc(final_date_result,final_full_res)
plot(roc_curve_fy2e, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.887
dat <- data.frame(truth = as.factor(final_date_result), pred_1 = as.numeric(final_full_res), pred_2 = 1-as.numeric(final_full_res))

pr_dat_fy2e <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_fy2e %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_fy2e$recall*pr_dat_fy2e$precision)/(pr_dat_fy2e$recall+pr_dat_fy2e$precision)
thre <- pr_dat_fy2e$.threshold[which.max(F1)]
normalized_thre <- (thre-min(final_full_res,na.rm = TRUE))/(max(final_full_res,na.rm = TRUE)-min(final_full_res,na.rm = TRUE))
fy2e_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_fy2e$recall[which.max(F1)],
  precision =pr_dat_fy2e$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
fy2e_model
```
...
ROC curve
```{r}
roc_curve_fy2f <- roc(final_date_result,final_full_res)
plot(roc_curve_fy2f, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.917
dat <- data.frame(truth = as.factor(final_date_result), pred_1 = as.numeric(final_full_res), pred_2 = 1-as.numeric(final_full_res))

pr_dat_fy2f <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_fy2f %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_fy2f$recall*pr_dat_fy2f$precision)/(pr_dat_fy2f$recall+pr_dat_fy2f$precision)
thre <- pr_dat_fy2f$.threshold[which.max(F1)]
normalized_thre <- (thre-min(final_full_res,na.rm = TRUE))/(max(final_full_res,na.rm = TRUE)-min(final_full_res,na.rm = TRUE))
fy2f_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_fy2f$recall[which.max(F1)],
  precision =pr_dat_fy2f$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
fy2f_model
```
...
ROC curve
```{r}
roc_curve_fy2h <- roc(final_date_result,final_full_res)
plot(roc_curve_fy2h, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.875
dat <- data.frame(truth = as.factor(final_date_result), pred_1 = as.numeric(final_full_res), pred_2 = 1-as.numeric(final_full_res))

pr_dat_fy2h <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_fy2h %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
  geom_point(x=0.3,y=0.333,col="red",pch=8)+
  geom_point(x=0.6,y=0.188,col="red",pch=8)+
  coord_equal()
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_fy2h$recall*pr_dat_fy2h$precision)/(pr_dat_fy2h$recall+pr_dat_fy2h$precision)
thre <- pr_dat_fy2h$.threshold[which.max(F1)]
normalized_thre <- (thre-min(final_full_res,na.rm = TRUE))/(max(final_full_res,na.rm = TRUE)-min(final_full_res,na.rm = TRUE))
fy2h_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_fy2h$recall[which.max(F1)],
  precision =pr_dat_fy2h$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
fy2h_model
```
...
ROC curve
```{r}
roc_curve_fy4a <- roc(final_date_result,final_full_res)
plot(roc_curve_fy4a, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.842
dat <- data.frame(truth = as.factor(final_date_result), pred_1 = as.numeric(final_full_res), pred_2 = 1-as.numeric(final_full_res))

pr_dat_fy4a <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_fy4a %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_fy4a$recall*pr_dat_fy4a$precision)/(pr_dat_fy4a$recall+pr_dat_fy4a$precision)
thre <- pr_dat_fy4a$.threshold[which.max(F1)]
normalized_thre <- (thre-min(final_full_res,na.rm = TRUE))/(max(final_full_res,na.rm = TRUE)-min(final_full_res,na.rm = TRUE))
fy4a_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_fy4a$recall[which.max(F1)],
  precision =pr_dat_fy4a$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
fy4a_model
```
...
roc_curve_srl <- roc(srl_ts_date,srl_ts_res)
plot(roc_curve_srl, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.947
dat <- data.frame(truth = as.factor(srl_ts_date), pred_1 = as.numeric(srl_ts_res), pred_2 = 1-as.numeric(srl_ts_res))

pr_dat_srl <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_srl %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_srl$recall*pr_dat_srl$precision)/(pr_dat_srl$recall+pr_dat_srl$precision)
thre <- pr_dat_srl$.threshold[which.max(F1)]
normalized_thre <- (thre-min(srl_ts_res,na.rm = TRUE))/(max(srl_ts_res,na.rm = TRUE)-min(srl_ts_res,na.rm = TRUE))
srl_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_srl$recall[which.max(F1)],
  precision =pr_dat_srl$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
srl_model
```
...
roc_curve_top <- roc(top_ts_date,top_ts_res)
plot(roc_curve_top, print.auc = TRUE, print.auc.x = 0.25, print.auc.y = 0.7, auc.polygon = TRUE)
# 0.696
dat <- data.frame(truth = as.factor(top_ts_date), pred_1 = as.numeric(top_ts_res), pred_2 = 1-as.numeric(top_ts_res))

pr_dat_top <- pr_curve(dat, truth, pred_1,event_level = "second")
pr_dat_top %>%
  arrange(.threshold) %>% # this step is not strictly necessary here because the rows are already ordered by `.threshold`
  ggplot() +
  geom_path(aes(recall, precision)) + 
...
calculate the F1, recall, precision
```{r}
F1 <- 2*(pr_dat_top$recall*pr_dat_top$precision)/(pr_dat_top$recall+pr_dat_top$precision)
thre <- pr_dat_top$.threshold[which.max(F1)]
normalized_thre <- (thre-min(top_ts_res,na.rm = TRUE))/(max(top_ts_res,na.rm = TRUE)-min(top_ts_res,na.rm = TRUE))
top_model <- tibble(
  threshold =normalized_thre,
  recall = pr_dat_top$recall[which.max(F1)],
  precision =pr_dat_top$precision[which.max(F1)],
  F1 = F1[which.max(F1)]
)
top_model
```
...
```{r}
color <-col_vector
plot(roc_curve_cs2,col=color[1],lwd=3)
par(new=TRUE)
plot(roc_curve_h2a,col=color[2],lwd=3)
par(new=TRUE)
plot(roc_curve_s3a,col=color[3],lwd=3)
par(new=TRUE)
plot(roc_curve_s3b,col=color[4],lwd=3)
par(new=TRUE)
plot(roc_curve_s6a,col=color[5],lwd=3)
par(new=TRUE)
plot(roc_curve_ja1,col=color[6],lwd=3)
par(new=TRUE)
plot(roc_curve_ja2,col=color[7],lwd=3)
par(new=TRUE)
plot(roc_curve_ja3,col=color[8],lwd=3)
par(new=TRUE)
plot(roc_curve_top,col=color[9],lwd=3)
par(new=TRUE)
plot(roc_curve_srl,col=color[10],lwd=3)
par(new=TRUE)
plot(roc_curve_fy2d,col=color[11],lwd=3)
par(new=TRUE)
plot(roc_curve_fy2e,col=color[12],lwd=3)
par(new=TRUE)
plot(roc_curve_fy2f,col=color[13],lwd=3)
par(new=TRUE)
plot(roc_curve_fy2h,col=color[14],lwd=3)
par(new=TRUE)
plot(roc_curve_fy4a,col=color[15],lwd=3)
legend(x = "topright",          # Position
       legend = c("CRYOSAT2", "Haiyang-2A","Sentinel-3A","Sentinel-3B","Sentinel-6A","Jason-1","Jason-2","Jason-3","TOPEX","SARAL","Fengyun-2D","Fengyun-2E","Fengyun-2F","Fengyun-2H","Fengyun-4A"),  # Legend texts          # Line types
       col =color,         # Line colors
       lwd = 2)  
```

pr_curve
```{r}
colors <- c("CRYOSAT2" = color[1], "Haiyang-2A" = color[2], "Sentinel-3A" = color[3],"Sentinel-3B"=color[4],"Sentinel-6A"=color[5],"Jason-1"=color[6],"Jason-2"=color[7],"Jason-3"=color[8],"TOPEX"=color[9],"SARAL"=color[10],"Fengyun-2D"=color[11],"Fengyun-2E"=color[12],"Fengyun-2F"=color[13],"Fengyun-2H"=color[14],"Fengyun-4A"=color[15])
ggplot()+
  geom_path(data=pr_dat_cs2,aes(recall, precision,color="CRYOSAT2"),size=1)+
  geom_path(data=pr_dat_h2a,aes(recall, precision,color="Haiyang-2A"),size=1)+
  geom_path(data=pr_dat_s3a,aes(recall, precision,color="Sentinel-3A"),size=1)+
  geom_path(data=pr_dat_s3b,aes(recall, precision,color="Sentinel-3B"),size=1)+
  geom_path(data=pr_dat_s6a,aes(recall, precision,color="Sentinel-6A"),size=1)+
  geom_path(data=pr_dat_ja1,aes(recall, precision,color="Jason-1"),size=1)+
  geom_path(data=pr_dat_ja2,aes(recall, precision,color="Jason-2"),size=1)+
  geom_path(data=pr_dat_ja3,aes(recall, precision,color="Jason-3"),size=1)+
  geom_path(data=pr_dat_top,aes(recall, precision,color="TOPEX"),size=1)+
  geom_path(data=pr_dat_srl,aes(recall, precision,color="SARAL"),size=1)+
  geom_path(data=pr_dat_fy2d,aes(recall, precision,color="Fengyun-2D"),size=1)+
  geom_path(data=pr_dat_fy2e,aes(recall, precision,color="Fengyun-2E"),size=1)+
  geom_path(data=pr_dat_fy2f,aes(recall, precision,color="Fengyun-2F"),size=1)+
  geom_path(data=pr_dat_fy2h,aes(recall, precision,color="Fengyun-2H"),size=1)+
  geom_path(data=pr_dat_fy4a,aes(recall, precision,color="Fengyun-4A"),size=1)+
    labs(x = "Time",
         color = "Legend") +
    scale_color_manual(values = colors)
```
